 ...
 **/
export async function getWidgets(widgetsDir, viteHandle) {
 ...
 */
export async function getWidgetHTML(widgetName, viteHandle) {
    ...
    if (isViteDevServer) {
        ...
        try {
            ...
            throw new Error(`Failed to load widget '${widgetName}' using Vite. ` + `Error: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    else {
        // Production: Read the built HTML file using Vite's manifest
        const options = (viteHandle || {});
    ...
    // Get explicit baseUrl from the handle
    const explicitBaseUrl = viteHandle.baseUrl;
    ...
    else if (explicitBaseUrl && isAbsoluteUrl(explicitBaseUrl)) {
        // Use the explicit baseUrl provided (and validate it's absolute)
    ...
    if (!effectiveBaseUrl) {
        throw new Error(`Widget HTML requires an absolute base URL for sandboxed iframes. ` +
            ...
            `Current Vite base: ${viteBase || "(not set)"}, provided baseUrl: ${explicitBaseUrl || "(not set)"}`);
    }
    // Validate that the effective base URL is actually absolute
    if (!isAbsoluteUrl(effectiveBaseUrl)) {
        throw new Error(`The resolved base URL "${effectiveBaseUrl}" is not an absolute URL. ` +
 ...
 */
export function generateWidgetEntrypointHTML(widgetName) {
...
export function chatGPTWidgetPlugin(options = {}) {
