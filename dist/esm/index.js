...
function transformHtmlWithAbsoluteUrls(html, baseUrl) {
    ...
    html = html.replace(/(<script[^>]+src=["'])\/([^"']+)(["'])/g, (_match, prefix, path, suffix) => `${prefix}${normalizedBase}${path}${suffix}`);
    // Transform link href attributes (for modulepreload, stylesheets, etc.)
    html = html.replace(/(<link[^>]+href=["'])\/([^"']+)(["'])/g, (_match, prefix, path, suffix) => `${prefix}${normalizedBase}${path}${suffix}`);
    return html;
}
/**
...
export function chatGPTWidgetPlugin(options = {}) {
    ...
    return {
        ...
        config(config) {
            ...
            // Conditionally include OpenAI packages in optimizeDeps if `@openai/apps-sdk-ui` is installed
            const openaiPackages = [
                "@openai/apps-sdk-ui/components/AppsSDKUIProvider",
                "@openai/apps-sdk-ui/components/Button",
                "@openai/apps-sdk-ui/components/Icon",
            ];
            const openaiPackagePath = path.resolve(viteRoot, "node_modules", "@openai/apps-sdk-ui");
            const hasOpenaiPackage = fsSync.existsSync(openaiPackagePath);
            const packagesToInclude = [...(hasGadgetPackage ? [gadgetPackage] : []), ...(hasOpenaiPackage ? openaiPackages : [])];
            return {
                optimizeDeps: {
                    include: packagesToInclude,
                },
            };
        },
